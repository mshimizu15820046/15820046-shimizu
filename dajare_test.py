...
if __name__=="__main__":
    ...
    #入力となる駄洒落データベース
    data = pd.read_csv("./dajare_database.csv", encoding="cp932")
...
def dajare(doc):
    ...
    if (type_tane == 1 or type_tane == 2) and (type_henkei == 1 or type_henkei == 2):
...
for i in range(len(data)):
          ...
          text_tane = data["tane"][i]
          text_henkei = data["henkei"][i]
          text_tane = str(text_tane)
          text_henkei = str(text_henkei)
          text = str(text)
          text_tane = re.sub("\<.+\>", "", text_tane)
          text_henkei = re.sub("\<.+\>", "", text_henkei)
          text = re.sub("\<.+\>", "", text)
          kara = ""
          threshold = 0
          q_tane_henkei = 0
          ...
          henkei_word = ""
          mora_henkei_word = ""
          text_tane = text_tane.translate(str.maketrans({'、': kara, '？': kara, '！': kara, '～': kara, '…': kara, '「':kara, '」': kara, '\u3000': kara}))
          text_henkei = text_henkei.translate(str.maketrans({'、': kara, '？': kara, '！': kara, '～': kara, '…': kara, '「':kara, '」': kara, '\u3000': kara}))
          data["tane"][i] = text_tane
          data["henkei"][i] = text_henkei
          print("入力:",text)          
          doc = nlp(text)
          ...
          if len(t_lst1) == 0 and len(tt_lst) == 0:  
            tt_lst = mora_lst[0]
          tane_res1 = tt_lst[0] #tt_lstの最初の形態素を種表現とする
...
# 種表現候補が分かった場合
            ...
            #henkei_tupleがある場合
              ...
              #完全一致、部分一致の自動評価
              if data["tane"][i] == tane_word:
                data["tane match"][i] = "True"
                data["tane match2"][i] = "True"
                type_tane = 1
              else:
                data["tane match"][i] = "False"
                if tane_word in data["tane"][i] or data["tane"][i] in tane_word:
                  data["tane match2"][i] = "True"
                  type_tane = 2
                else:
                  data["tane match2"][i] = "False"

              if data["henkei"][i] == henkei_word:
                data["henkei match"][i] = "True"
                data["henkei match2"][i] = "True"
                type_henkei = 1
              else:
                data["henkei match"][i] = "False"
                h_words2 = jaconv.hira2kata(henkei_word) #ひらがなをカタカナに変換
                henkei_word = h_words2
                data["henkei extraction"][i] = henkei_word
                if data["henkei"][i] == henkei_word:
                  data["henkei match"][i] = "True"
                  data["henkei match2"][i] = "True"
                  ...
                  data["henkei match"][i] = "False"
                  h_words3 = jaconv.kata2hira(henkei_word) #カタカナをひらがなに変換
                  henkei_word = h_words3
                  data["henkei extraction"][i] = henkei_word
                  if henkei_word in data["henkei"][i] or data["henkei extraction"][i] in henkei_word:
                    data["henkei match2"][i] = "True"
                    type_henkei = 2
                  else:
                    data["henkei match2"][i] = "False"
    ...
    #henkei_listを取得
          ...
          #もし変形表現をうまくとりだせず、完全一致の形態素を出力する場合
              ...
              if data["tane"][i] == tane_word:
                ...
                data["tane match"][i] = "False"
                if tane_word in data["tane extraction"][i] or data["tane"][i] in tane_word:
                  data["tane match2"][i] = "True"
                  type_tane = 2
                else:
                  data["tane match2"][i] = "False"
              ...
              if data["henkei"][i] == henkei_word:
                ...
                if data["henkei"][i] == henkei_word:
                  ...
                  henkei_word = h_words3
                  data["henkei extraction"][i] = henkei_word
                  if henkei_word in data["henkei extraction"][i] or data["henkei extraction"][i] in henkei_word:
                    data["henkei match2"][i] = "True"
                    type_henkei = 2
                  else:
                    data["henkei match2"][i] = "False"
          ...
          if henkei_word == "None":
                data["henkei match2"][i] = "False"
                type_henkei = 0
          if (type_tane == 1 or type_tane == 2) and (type_henkei == 1 or type_henkei == 2) and (thres == "最適な音韻類似度"):
...
with StyleFrame.ExcelWriter("./dajare_output.xlsx") as writer:
      # DataFrame/dfをStyleFrameクラスの引数にしてインスタンス生成
      sf = StyleFrame(df)
      sf.set_column_width(columns = 2, width = 55)
      sf.set_column_width(columns = 3, width = 30)
      sf.set_column_width(columns = 4, width = 30)
      sf.set_column_width(columns = 5, width = 30)
      sf.set_column_width(columns = 6, width = 30)
      sf.set_column_width(columns = 7, width = 20)
      sf.set_column_width(columns = 8, width = 20)
      sf.set_column_width(columns = 9, width = 20)
      sf.set_column_width(columns = 10, width = 20)
      sf.set_column_width(columns = 11, width = 20)
      sf.set_column_width(columns = 12, width = 20)
      #tyleFrameのto_excelメソッドで書き込む
      sf.to_excel(writer, index = False, sheet_name = 'dajare3333')
